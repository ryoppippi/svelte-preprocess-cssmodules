 ...
 */
function encodeBufferToBase(buffer: Buffer, base: number): string {
 ...
 */
function getHashDigest(buffer: Buffer, hashType: string, digestType: string, maxLength = 9999): string {
	...
	if (
		...
		return encodeBufferToBase(hash.digest(), Number.parseInt(digestType.substr(4), 10)).substr(
			0,
			maxLength,
		);
	}
