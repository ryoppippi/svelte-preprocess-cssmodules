...
export function normalizeIncludePaths(paths: string[]): string[] {
	return paths.map(includePath => normalizePath(path.resolve(includePath)));
}
 ...
 */
export function isFileIncluded(includePaths: string[], filename: string): boolean {
 ...
 * @returns The status
 */
export function hasModuleImports(content: string): boolean {
	const pattern = /(?<!\/\/\s*)import\s*(?:(.+)\s+from\s+)?['|"](.+?(module\.s?css))['|"];?/g;
